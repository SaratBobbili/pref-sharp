...
def tokenize_with_chat_template(tokenizer, prompts, use_chat_template, device):
    if use_chat_template:
        ...
        inputs = tokenizer(formatted_prompts, padding=True, return_tensors="pt").to(
            device
        )
...
def get_output_indices(outputs, eos_token_id):
    # return the index of the first eos_token per sequence, or last token if none
    bs, seq_len = outputs.shape
    device = outputs.device
    # default to the last token if no eos token is found
    outputs_end_indices = torch.full(
        (bs,), seq_len - 1, dtype=torch.long, device=device
    )
    # find all eos token positions
    eos_positions = (outputs == eos_token_id).nonzero(as_tuple=False)
    for batch_idx, pos in eos_positions.tolist():
        # take the earliest eos position
        if pos < outputs_end_indices[batch_idx].item():
            outputs_end_indices[batch_idx] = pos
    return outputs_end_indices


def create_classifier_data(
    all_data, use_all_ref_tokens, drop_no_variation, max_length=None
):
    ...
    for i in tqdm(range(len(all_data))):
        assert (
            len(all_data[i][prompt_key])
            == len(all_data[i][response_key])
            == len(all_data[i][reward_key])
        )
        ...
        for j in range(len(all_data[i][prompt_key])):
            ...
            if use_all_ref_tokens == 0:
                ...
                target_ids = [all_data[i][prompt_key][j][-1]] + all_data[i][
                    response_key
                ][j]
            num_token_full_response = (
                all_data[i][num_token_prompt_key][j] + len(target_ids) - 1
            )
...
def kl_divergence(logits1, logits2):
    assert (
        logits1.shape == logits2.shape
    ), f"Shapes of logits1 and logits2 must match: {logits1.shape} vs. {logits2.shape}"
...
class DynamicBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        for idx in indices:
            ...
            current_num_tokens = len(current_item["input_ids"]) + len(
                current_item["target_ids"]
            )
            ...
            if (
                ...
                assert (
                    len(batch) > 0
                ), "effective batch size is 0, max_tokens_per_batch of {0} is too small for {1} tokens".format(
                    self.max_tokens_per_batch, current_num_tokens
                )
                yield batch
                batch = []
                batch_max_length = current_num_tokens
...
def save_model(
    ...
    if accelerator.is_main_process:
        ...
        if lr_scheduler is not None:
            torch.save(
                lr_scheduler.state_dict(), os.path.join(save_dir, "lr_scheduler.pt")
            )

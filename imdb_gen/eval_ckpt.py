...
parser.add_argument(
    "--classifier_ckpt_path", required=True, type=str, help="a ckpt path"
)
parser.add_argument(
    "--inference_mode",
    default=None,
...
parser.add_argument(
    "--kl_batch_size", default=2, type=int, help="batch size for KL computation"
)
parser.add_argument(
    "--num_samples", default=8, type=int, help="number of samples per problem"
)
parser.add_argument(
    "--cd_baseline", default=0, type=int, help="whether to use CD baseline"
)
parser.add_argument(
    "--use_chat_template",
    default=None,
...
parser.add_argument(
    ...
    default=-1,
    type=int,
    help="max tokens for prompt, -1 means no limit",
)
parser.add_argument(
    "--max_new_tokens", default=1024, type=int, help="max tokens for sampling 1024"
)
parser.add_argument(
...
parser.add_argument(
    "--num_atoms", default=None, type=int, help="number of atoms for mle classifier"
)
parser.add_argument(
    "--V_min", default=None, type=float, help="V_min for histogram learning"
)
parser.add_argument(
    "--V_max", default=None, type=float, help="V_max for histogram learning"
)
parser.add_argument(
    "--shift_reward",
    default=None,
    type=float,
    help="shift reward by value (subtraction)",
)
parser.add_argument(
    "--scale_reward",
    default=None,
...
# For IMDb evaluation, default the match_fn_type to strict
if isinstance(dataset_type, str) and dataset_type.lower() == "imdb":
    args_dict["match_fn_type"] = "strict"
...
# Only resolve train_eval_save_path for non-IMDb evaluations
if dataset_type.lower() != "imdb":
    train_eval_save_path = resolve_dict_value(
        args_dict, training_args_dict, "train_eval_save_path"
    )
else:
    train_eval_save_path = None
batch_size = args.batch_size
kl_batch_size = args.kl_batch_size
num_samples = args.num_samples
cd_baseline = args.cd_baseline
use_chat_template = resolve_dict_value(
    args_dict, training_args_dict, "use_chat_template"
)
...
max_prompt_length = resolve_dict_value(
    args_dict, training_args_dict, "max_prompt_length"
)
...
individual_eval_inference_output_dir = os.path.join(
    output_dir,
    "individual_eval_inference_eta_{0}_top_k_{1}_temp_{2}".format(
        eta, top_k, temperature
    ),
)
...
if (
    ...
    and os.path.exists(
        os.path.join(
            output_dir,
            "inference_eval_results_eta_{0}_top_k_{1}_temp_{2}.jsonl".format(
                eta, top_k, temperature
            ),
        )
    )
    and os.path.exists(
        os.path.join(
            output_dir,
            "reward_stats_eta_{0}_top_k_{1}_temp_{2}.json".format(
                eta, top_k, temperature
            ),
        )
    )
...
if dataset_type == "gsm8k":
    ...
    # IMDb evaluation, skip standard dataset setup
    pass
else:
...
# Load evaluation prompts for non-IMDb datasets
if dataset_type.lower() != "imdb":
    ...
    # IMDb: inference_eval_examples unused, use custom prompts
    inference_eval_examples = []
...
# If evaluating on IMDb, run single-generation test prompt evaluation with KL and exit
if dataset_type.lower() == "imdb":
    # Load 5000 prompts from IMDb test split
    dataset = load_dataset(data_path, split="test")
    prompts = []
    for idx, item in enumerate(dataset):
        if idx >= 5000:
            break
        tokenized = tokenizer(item["text"], truncation=True, max_length=60)
        prompts.append(
    ...
    sentiment_tokenizer = AutoTokenizer.from_pretrained(reward_name)
    results = []
    # Prepare for guided generation and KL computation
    ...
    for batch_start in tqdm(range(0, len(prompts), batch_size)):
        ...
        # Generate sequences and capture guided logits
        current_outputs = generate_with_classifier_guidance(
            ref_model, tokenizer, logit_processor, inputs, generate_kwargs, True, False
        )
        sequences = current_outputs["sequences"]
        generations = tokenizer.batch_decode(sequences, skip_special_tokens=True)
        # Stack guided logits
        scores = tuple(e.cpu() for e in current_outputs["scores"])
        aligned_model_scores = torch.stack(scores, dim=1).float()
        # Compute trajectory KL divergence
        token_kl_list = []
        for k in range(0, sequences.size(0), kl_batch_size):
            end = k + kl_batch_size
            mask = (sequences[k:end] != tokenizer.pad_token_id).long()
            concat_ids = torch.cat(
                [inputs["input_ids"][k:end], sequences[k:end]], dim=1
            )
            concat_mask = torch.cat([inputs["attention_mask"][k:end], mask], dim=1)
            ref_out = ref_model(input_ids=concat_ids, attention_mask=concat_mask)
            ref_logits = ref_out.logits[:, inputs["input_ids"].shape[1] - 1 : -1]
            ref_logits = ref_logits.float() / temperature
            kl_batch = utils.kl_divergence(
                aligned_model_scores[k:end].to(ref_logits.device), ref_logits
            )
            token_kl_list.append(kl_batch * mask)
        token_kl = torch.cat(token_kl_list, dim=0)
        traj_kl_list = token_kl.sum(dim=1).cpu().tolist()
        # Score sentiment and assemble results
        for prompt, gen, kl_val in zip(batch_prompts, generations, traj_kl_list):
            sent_in = sentiment_tokenizer(
                gen, return_tensors="pt", truncation=True, padding=True
            ).to(device)
            with torch.no_grad():
                logits = sentiment_model(**sent_in).logits
    ...
    write_jsonl(results, os.path.join(output_dir, "imdb_eval_results.jsonl"))
    # Compute and save summary statistics for sentiment and KL
    sentiment_scores = [r["sentiment_score"] for r in results]
    traj_kls = [r["traj_kl"] for r in results]
    stats = {
        "avg_sentiment_score": float(np.mean(sentiment_scores)),
        "avg_traj_kl": float(np.mean(traj_kls)),
    }
    with open(os.path.join(output_dir, "imdb_eval_stats.json"), "w") as sf:
        json.dump(stats, sf)
    exit(0)
...
for i in range(num_samples):
    ...
    if not force:
        existing_data_paths = glob.glob(
            os.path.join(
                individual_eval_inference_output_dir, "*_r{0}.json".format(repeat_index)
            )
        )
        existing_indices = [
            int(os.path.basename(path).split("_")[0]) for path in existing_data_paths
        ]
    else:
        existing_indices = []
    data_to_infer = []
    ...
    print(
        "total number of problems to infer for repeat {0}:".format(repeat_index),
        len(data_to_infer),
    )
    ...
    for j in tqdm(range(num_batches)):
        ...
        current_prompts = [
            data_to_infer[k]["prompt"]
            for k in range(batch_start_index, batch_end_index)
        ]
        current_inputs, current_formatted_prompts = tokenize_with_chat_template(
            tokenizer, current_prompts, use_chat_template, device
        )
        generate_kwargs["output_scores"] = True
        generate_kwargs["return_dict_in_generate"] = True
        current_outputs = generate_with_classifier_guidance(
        ...
        current_outputs_text = tokenizer.batch_decode(
            current_outputs_id, skip_special_tokens=True
        )
        current_outputs["scores"] = tuple(
            [e.cpu() for e in current_outputs["scores"]]
        )  # prevent OOM
        ...
        for k in range(0, len(batch_indices), kl_batch_size):
            ...
            output_attention_mask = (
                current_outputs_id[k : k + kl_batch_size] != tokenizer.pad_token_id
            ).long()
            concat_input_ids = torch.cat(
                [
                    current_inputs["input_ids"][k : k + kl_batch_size],
                    current_outputs_id[k : k + kl_batch_size],
                ],
                dim=1,
            )
            concat_attention_mask = torch.cat(
                [
                    current_inputs["attention_mask"][k : k + kl_batch_size],
                    output_attention_mask,
                ],
                dim=1,
            )
            ...
            ref_model_output_logits = ref_model_output.logits[
                :, current_inputs["input_ids"].shape[1] - 1 : -1
            ]
            ref_model_output_logits = ref_model_output_logits.float() / temperature
            del ref_model_output
            torch.cuda.empty_cache()

            cur_token_kl = utils.kl_divergence(
                aligned_model_scores[k : k + kl_batch_size].to(
                    ref_model_output_logits.device
                ),
                ref_model_output_logits,
            )
        ...
        for k in range(len(batch_indices)):
            original_problem_id = train_eval_problems_d[
                data_to_infer[batch_indices[k]]["problem"]
            ]["id"]
            eval_problem_id = original_id_to_eval_id_d[original_problem_id]
            current_output_path = os.path.join(
                individual_eval_inference_output_dir,
                f"{eval_problem_id}_r{repeat_index}.json",
            )
            assert not os.path.exists(
                current_output_path
            ), f"expect {current_output_path} to not exist"
...
for i in range(len(inference_eval_examples)):
    ...
    for j in range(num_samples):
        current_output_path = os.path.join(
            individual_eval_inference_output_dir, f"{i}_r{j}.json"
        )
        assert os.path.exists(
            current_output_path
        ), f"expect {current_output_path} to exist"
        if j == 0:
        ...
        inference_eval_examples[i][prediction_key].append(
            current_prediction_data["prediction"]
        )
        inference_eval_examples[i]["token_kl"].append(
            current_prediction_data["token_kl"]
        )
...
for i in range(len(inference_eval_examples)):
    ...
    if dataset_type == "MATH":
        ...
        answer_processed = process_sample(
            solution_or_answer, None, extract_last_occurrence
        )
    else:
        answer_processed = solution_or_answer
    all_samples = inference_eval_examples[i][prediction_key]
    all_processed_predictions = [
        process_sample(sample, None, extract_last_occurrence) for sample in all_samples
    ]
    predictions_correctness = [
        match_fn(sample, answer_processed) if sample is not None else False
    ...
    majority_vote_correct = compute_majority_vote_correct(
        all_processed_predictions,
        predictions_correctness,
        sample_partition,
...
single_sample_accuracy_list = get_average_reward(
    inference_eval_examples, "predictions_correctness", 100
)
single_sample_accuracy_mean = np.mean(single_sample_accuracy_list)
majority_vote_accuracy_mean = np.mean(
    [example["majority_vote_correct"] for example in inference_eval_examples]
)
pass_k_accuracy_mean = np.mean(
    [example["pass_at_k"] for example in inference_eval_examples]
)

with open(
    os.path.join(
        output_dir,
        "reward_stats_eta_{0}_top_k_{1}_temp_{2}.json".format(eta, top_k, temperature),
    ),
    "w",
) as f:
    json.dump(

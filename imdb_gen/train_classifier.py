...
parser.add_argument(
    "--world_size", default=1, type=int, help="number of processes / gpus"
)
parser.add_argument(
    "--ref_model_id",
...
parser.add_argument(
    ...
    default="mle",
    type=str,
    help="loss type for the classifier, bce or mse",
)
parser.add_argument(
    "--use_bias",
    default=0,
...
parser.add_argument(
    ...
    default="imdb",
    type=str,
    help="dataset type (gsm8k, math, imdb)",
)
parser.add_argument(
    "--data_paths",
    default=["collected_data/all_train_data_with_learnt_rewards.jsonl"],
    nargs="+",
    type=str,
    help="all paths to the combined training data JSONL",
)
parser.add_argument(
    "--batch_size",
    default=8,
    type=int,
    help="batch size for training classifier (max allowed)",
)
parser.add_argument(
    "--max_batch_num_tokens",
    default=-1,
    type=int,
...
parser.add_argument(
    "--shift_reward", default=0, type=float, help="shift reward by value (subtraction)"
)
parser.add_argument(
    "--scale_reward",
    default=1,
    type=float,
    help="scale reward by value (multiplication)",
)
parser.add_argument(
    "--cd_baseline", default=0, type=int, help="if 1, run the CD baseline."
)
parser.add_argument(
    "--use_chat_template",
    default=0,
...
parser.add_argument(
    ...
    default="bfloat16",
    type=str,
    help="data type for the model bfloat16 or empty string",
)
parser.add_argument(
    "--temperature", default=0.8, type=float, help="temperature for sampling"
)
parser.add_argument(
    "--use_all_ref_tokens",
    default=1,
...
parser.add_argument(
    ...
    default=0.1,
    type=float,
    help="ratio of id samples for evaluation",
)
parser.add_argument(
    "--eta",
    default=1,
...
parser.add_argument(
    "--top_k", type=int, default=-1, help="top k logits to modify, -1 means all logits"
)
parser.add_argument(
    "--output_dir",
    default="checkpoints/tinyllama_imdb_data/",
    type=str,
...
parser.add_argument(
    "--num_epochs", default=5, type=int, help="number of epochs for training"
)
parser.add_argument(
    "--eval_max_size", default=1000, type=int, help="number of epochs for training"
)
parser.add_argument(
    "--lr", default=2e-5, type=float, help="learning rate for the classifier"
)
parser.add_argument(
    "--warmup_step",
    default=-1,
    type=int,
    help="warmup steps for the classifier, -1 means no warmup",
)
parser.add_argument(
    "--weight_decay", default=1e-2, type=float, help="weight decay for the classifier"
)
...
parser.add_argument(
    ...
    default=1,
    type=int,
    help="whether to save optimizer and scheduler state",
)
parser.add_argument("--seed", default=47, type=int, help="seed for reproduction")
parser.add_argument("--track", default=1, type=int, help="whether to report to wandb")
parser.add_argument("--wandb_entity", default=None, type=str, help="wandb entity")
parser.add_argument(
    "--wandb_project",
    default="qsharp_classifiers_imdb",
    type=str,
    help="wandb project",
)
parser.add_argument(
    "--wandb_run_name",
    default="imdb_data_mle",
    type=str,
    help="wandb run name",
)
parser.add_argument(
...
parser.add_argument(
    ...
    default=None,
    type=float,
    help="V_min for histogram learning",
)
parser.add_argument(
    "--V_max",
    default=None,
    type=float,
    help="V_max for histogram learning",
)
parser.add_argument(
    "--max_length", default=-1, type=int, help="max tokens for training"
)
...
# Auto-resume: if no classifier_ckpt_path provided, but existing checkpoints under output_dir, resume from latest
ckpt_dirs = (
    glob.glob(os.path.join(args.output_dir, "ckpt_*"))
    if hasattr(args, "output_dir")
    else []
)
if args.classifier_ckpt_path is None and ckpt_dirs:
    # pick the checkpoint with largest step number
    latest = max(ckpt_dirs, key=lambda x: int(os.path.basename(x).split("_")[-1]))
    print(f"Resuming from checkpoint {latest}")
    args.classifier_ckpt_path = latest
    args.resume_opt_scheduler = 1
...
if classifier_ckpt_path is None:
    ...
    assert (
        resume_opt_scheduler is not None
    ), "resume_opt_scheduler must be specified when classifier_ckpt_path is not None"
...
if tokenizer.pad_token is None:
    ...
    tokenizer.pad_token = tokenizer.added_tokens_decoder[
        128002
    ].content  # reserved special token 0
...
if dataset_type.lower() == "gsm8k":
    ...
    answer_key = "solution"
    reward_key = "partial_guided_predictions_correctness"
elif dataset_type.lower() == "imdb":
    dataset_type = "IMDB"
    # reward_key = "partial_guided_rewards" # Q#
    # reward_key = "partial_guided_learnt_reward" # Q#HF
    reward_key = "partial_guided_vs_fully_pref"  # PITA
else:
...
for data_path in data_paths:
    ...
    for i in range(len(current_data)):
        idx = current_data[i].get("idx")
        if idx not in problem_position_d:
            problem_position_d[idx] = len(all_data)
            all_data.append(current_data[i])
        else:
            for k in merge_keys:
                all_data[problem_position_d[idx]][k].extend(current_data[i].get(k, []))
...
for i in range(len(all_data)):
    ...
    # filter out None entries and keep prompts/responses in sync
    rewards_list = current_data.get(reward_key, [])
    filtered_prompts_tok = []
    filtered_resp_tok = []
    filtered_num_tokens = []
    filtered_rewards = []
    skipped_none = 0
    for r, ptok, rtok, num_tok in zip(
        rewards_list,
        current_data["partial_guided_prompts_tokenized"],
        current_data["partial_guided_responses_tokenized"],
        current_data["num_response_tokens_in_partial_guided_prompts"],
    ):
        if r is None:
            skipped_none += 1
            continue
        filtered_rewards.append(float(r))
        filtered_prompts_tok.append(ptok)
        filtered_resp_tok.append(rtok)
        filtered_num_tokens.append(num_tok)
    if skipped_none:
        print(
            f"Example idx={current_data.get('idx', i)}: skipped {skipped_none} None rewards"
        )
    # overwrite with filtered lists
    current_data["partial_guided_prompts_tokenized"] = filtered_prompts_tok
    current_data["partial_guided_responses_tokenized"] = filtered_resp_tok
    current_data["num_response_tokens_in_partial_guided_prompts"] = filtered_num_tokens
    current_data["reward"] = filtered_rewards
...
if loss_type == "bce":
    ...
    assert (
        min_reward >= 0 and max_reward <= 1
    ), f"min reward: {min_reward}, max reward: {max_reward} should be in [0, 1] for bce loss."

# random split train vs eval directly
indices = np.random.permutation(len(all_data))
split_pt = int((1 - id_eval_ratio) * len(indices))
train_idx, eval_idx = indices[:split_pt], indices[split_pt:]
all_train_data = [all_data[i] for i in train_idx]
all_eval_data = [all_data[i] for i in eval_idx]
...
train_classifier_data = {
    k: [all_train_classifier_data[k][i] for i in train_indices]
    for k in all_train_classifier_data
}
id_eval_classifier_data = {
    k: [all_train_classifier_data[k][i] for i in id_eval_indices]
    for k in all_train_classifier_data
}
ood_eval_classifier_data = create_classifier_data(
    all_eval_data, 1, False, args.max_length
)  # prevent evaluation being biased by favoring the later tokens

if eval_max_size != -1:
    if eval_max_size < len(id_eval_classifier_data["input_ids"]):
        id_eval_random_indices = np.random.choice(
            len(id_eval_classifier_data["input_ids"]), eval_max_size, replace=False
        )
        id_eval_classifier_data = {
            k: [id_eval_classifier_data[k][i] for i in id_eval_random_indices]
            for k in id_eval_classifier_data
        }
        print("id_eval indices sum", np.sum(id_eval_random_indices))
    if eval_max_size < len(ood_eval_classifier_data["input_ids"]):
        ood_eval_random_indices = np.random.choice(
            len(ood_eval_classifier_data["input_ids"]), eval_max_size, replace=False
        )
        ood_eval_classifier_data = {
            k: [ood_eval_classifier_data[k][i] for i in ood_eval_random_indices]
            for k in ood_eval_classifier_data
        }
...
if max_batch_num_tokens == -1:
    ...
    id_eval_classifier_loader = DataLoader(
        id_eval_classifier_dataset,
        batch_size=batch_size,
        shuffle=False,
        drop_last=True,
        num_workers=num_workers,
    ...
    ood_eval_classifier_loader = DataLoader(
        ood_eval_classifier_dataset,
        batch_size=batch_size,
        shuffle=False,
        drop_last=True,
        num_workers=num_workers,
    ...
    train_sampler = DynamicBatchSampler(
        train_classifier_dataset, batch_size, max_batch_num_tokens, shuffle=True
    )
    id_eval_sampler = DynamicBatchSampler(
        id_eval_classifier_dataset, batch_size, max_batch_num_tokens, shuffle=False
    )
    ood_eval_sampler = DynamicBatchSampler(
        ood_eval_classifier_dataset, batch_size, max_batch_num_tokens, shuffle=False
    )
    train_classifier_loader = DataLoader(
        train_classifier_dataset,
    ...
    ood_eval_classifier_loader = DataLoader(
        ood_eval_classifier_dataset,
        batch_sampler=ood_eval_sampler,
        num_workers=num_workers,
        collate_fn=custom_collate_fn,
        pin_memory=True,
    )
...
if args.classifier_type == "Q":
    if init_mode == "zero":
        print(
            "before loading score weight mean",
            classifier_model.score.weight.data.mean().item(),
        )
        classifier_model.zero_init_classifier()
        print(
            "after loading score weight mean",
            classifier_model.score.weight.data.mean().item(),
        )
    elif init_mode == "reuse":
        temp_model = AutoModelForCausalLM.from_pretrained(
            classifier_model_id, **model_loading_kwargs, device_map="cpu"
        )
        ...
        if loss_type in ["mle", "qr"]:
            classifier_model.score.weight.data = lm_head_parameters.repeat(
                1, args.num_atoms
            ).view(vocab_size * args.num_atoms, -1)
        else:
...
optimizer = torch.optim.AdamW(
    classifier_model.parameters(), lr=lr, weight_decay=weight_decay
)
scheduler = get_constant_schedule_with_warmup(optimizer, num_warmup_steps=warmup_step)
if resume_opt_scheduler == 1:
    optimizer_scheduler_state = torch.load(
        os.path.join(classifier_ckpt_path, "optimizer_scheduler_state.pt")
    )
...
(
    ...
    id_eval_classifier_loader,
    ood_eval_classifier_loader,
    scheduler,
) = accelerator.prepare(
    classifier_model,
    optimizer,
...
for epoch in range(num_epochs):
    ...
    bar = (
        tqdm(train_classifier_loader)
        if accelerator.is_local_main_process
        else train_classifier_loader
    )
    torch.cuda.empty_cache()
    for batch_input_data in bar:
        ...
        loss = (
            outputs.loss / gradient_accumulation_step
        ...
        if (global_step // world_size + 1) % gradient_accumulation_step == 0:
            if args.classifier_type == "Q":
                ...
                grad_norm = accelerator.clip_grad_norm_(
                    classifier_model.parameters(), max_norm=5.0
                )
            optimizer.step()
            scheduler.step()
        ...
        if eval_freq != -1 and (global_step % eval_freq == 0 or global_step == 1):
            classifier_model.eval()
            unwrapped_classifier_model = (
                classifier_model.module
                if hasattr(classifier_model, "module")
            ...
            with torch.no_grad():
                ...
                # ID eval
                for eval_key, eval_loader in [
                    ("id", id_eval_classifier_loader),
                    ("ood", ood_eval_classifier_loader),
                ]:
                    for batch_input_data in eval_loader:
                        # always use ones during eval
                        batch_input_data["loss_weights"] = torch.ones_like(
                            batch_input_data["loss_weights"]
                        )
                        outputs = classifier_model(
                            input_ids=batch_input_data["input_ids"],
                            attention_mask=batch_input_data["attention_mask"],
                        ...
                        # compute predictions and flatten using loss_mask
                        cur_predictions = (
                            unwrapped_classifier_model.calculate_predictions(
                                outputs.logits
                            )
                        )  # [bs, seqlen]
                        mask = batch_input_data["loss_mask"]  # bool tensor [bs, seqlen]
                        preds_flat = cur_predictions[
                            mask
                        ]  # 1D tensor of valid positions
                        eval_predictions[eval_key].append(preds_flat)
                        # repeat rewards and flatten same mask
                        labels_expanded = (
                            batch_input_data["rewards"]
                            .unsqueeze(1)
                            .repeat(1, cur_predictions.size(1))
                        )
                        labels_flat = labels_expanded[mask]
                        eval_labels[eval_key].append(labels_flat)
                        if loss_type == "mle":
                            mle_stats = calculate_mle_stats(
                                outputs.logits, unwrapped_classifier_model.atoms
                            )  # mapping to [bs, seqlen]
                            for mle_key in mle_stats:
                                mle_stats[mle_key] = mask_stats(
                                    mle_stats[mle_key], batch_input_data["loss_mask"]
                                )
            ...
            eval_losses = {
                k: torch.mean(accelerator.gather(torch.tensor(v)))
                for k, v in eval_losses.items()
            }
            # flatten lists of 1D tensors for predictions and labels
            for k in eval_predictions.keys():
                eval_predictions[k] = torch.cat(eval_predictions[k])
                eval_labels[k] = torch.cat(eval_labels[k])
            # flatten MLE stats if needed
            if loss_type == "mle":
                for k in list(eval_stats.keys()):
                    merged = {}
                    for stat in eval_stats[k][0].keys():
                        # each x[stat] is [bs, seqlen] with masked entries = -1
                        flat_batches = [
                            flatten_and_remove_masked_elems(x[stat])
                            for x in eval_stats[k]
                        ]
                        merged[stat] = torch.cat(flat_batches)
                    eval_stats[k] = merged
            else:
                eval_stats = {}
            run.log(
                {
                    "ID Eval Loss": eval_losses["id"],
                    "ID Eval Explained Variance": calculate_explained_variance(
                        eval_predictions["id"], eval_labels["id"]
                    ),
                    "ID Eval R^2": calculate_r2(
                        eval_predictions["id"], eval_labels["id"]
                    ),
                    ...
                    "OOD Eval Explained Variance": calculate_explained_variance(
                        eval_predictions["ood"], eval_labels["ood"]
                    ),
                    "OOD Eval R^2": calculate_r2(
                        eval_predictions["ood"], eval_labels["ood"]
                    ),
            ...
            if loss_type == "mle":
                for k in ["id", "ood"]:
                    eval_stats[k] = {
                        stat: torch.mean(eval_stats[k][stat]) for stat in eval_stats[k]
                    }
                run.log(
                    {
                        "ID Eval MLE Stats": eval_stats["id"],
                        "OOD Eval MLE Stats": eval_stats["ood"],
                    },
                    step=global_step,
                )
            ...
            if inference_mode == "bernoulli":
                ...
                id_eval_rounded_predictions = [
                    float(eval_predictions["id"][i] > 0.5)
                    for i in range(len(eval_predictions["id"]))
                ]
                ood_eval_rounded_predictions = [
                    float(eval_predictions["ood"][i] > 0.5)
                    for i in range(len(eval_predictions["ood"]))
                ]
                id_eval_accuracy = np.mean(
                    np.array(id_eval_rounded_predictions)
                    == np.array(eval_labels["id"].cpu())
                )
                ood_eval_accuracy = np.mean(
                    np.array(ood_eval_rounded_predictions)
                    == np.array(eval_labels["ood"].cpu())
                )
                id_eval_roc_auc = roc_auc_score(
                    np.array(eval_labels["id"].cpu()),
                    np.array(eval_predictions["id"].cpu()),
                )
                ood_eval_roc_auc = roc_auc_score(
                    np.array(eval_labels["ood"].cpu()),
                    np.array(eval_predictions["ood"].cpu()),
                )
                run.log(
                    {
                        "ID Eval Accuracy": id_eval_accuracy,
                        "ID Eval ROC-AUC": id_eval_roc_auc,
                        "OOD Eval Accuracy": ood_eval_accuracy,
                        "OOD Eval ROC-AUC": ood_eval_roc_auc,
                    },
                    step=global_step,
                )
            classifier_model.train()

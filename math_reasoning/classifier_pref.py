...
class CustomLlamaForSequenceClassification(LlamaPreTrainedModel):
        ...
        if loss_type == "mse":
            ...
            if self.classifier_type == "Q":
                self.score = nn.Linear(
                    config.hidden_size, self.num_labels, bias=use_bias
                )
            ...
            if self.classifier_type == "Q":
                self.score = nn.Linear(
                    config.hidden_size, self.num_labels * self.num_atoms, bias=use_bias
                )
            elif self.classifier_type == "V":
                self.score = nn.Linear(
                    config.hidden_size, self.num_atoms, bias=use_bias
                )
        else:
    ...
    def calculate_loss(self, logits, labels, loss_weights, loss_mask):
        ...
        if self.loss_type == "mse":
            ...
            loss = self.loss_fct(
                relevant_logits, labels_expanded.to(relevant_logits.dtype)
            )
    ...
    def forward(
        ...
        if self.classifier_type == "Q":
            if labels is not None:
                ...
                logits = self.score(
                    hidden_states
                )  # [bs, seqlen, vocab_size] or [bs, seqlen, vocab_size * num_atoms]
                if self.loss_type in ["mse", "bce"]:
                    logits = logits.unsqueeze(-1)  # [bs, seqlen, vocab_size, 1]
                elif self.loss_type == "mle":
                    logits = logits.view(
                        bs, seqlen, self.num_labels, self.num_atoms
                    )  # [bs, seqlen, vocab_size, num_atoms]
                indexed_logits = logits[:, :-1][
                    torch.arange(bs)[:, None],
                    torch.arange(seqlen - 1),
                    input_ids[:, 1:],
                ]
                indexed_logits = indexed_logits.float()
                loss = self.calculate_loss(
                    indexed_logits, labels, loss_weights, loss_mask[:, 1:]
                )
                return SequenceClassifierOutputWithPast(
                    loss=loss, logits=indexed_logits
                )
            else:
                # inference
                ...
                hidden_states = transformer_outputs[
                    0
                ]  # [bs, seqlen, hidden_dim] or [bs, 1, hidden_dim]
                hidden_states = hidden_states[:, -1]  # [bs, hidden_size]
                logits = self.score(
                    hidden_states
                )  # [bs, vocab_size * (1 or num_atoms or 2)]
                ...
                if self.loss_type in ["mse", "bce"]:
                    ...
                    if logit_indices is not None:
                        ...
                        offsets = torch.arange(
                            self.num_atoms, device=logit_indices.device
                        )
                        expanded = (
                            logit_indices.unsqueeze(-1) * self.num_atoms + offsets
                        )  # [bs, top_k, num_atoms]
                        expanded = expanded.view(bs, -1)  # [bs, top_k * num_atoms]
                        logits = logits[
                            torch.arange(bs)[:, None], expanded
                        ]  # [bs, top_k * num_atoms]
                    logits = logits.float().view(
                        bs, top_k, self.num_atoms
                    )  # [bs, top_k, num_atoms]
            ...
            if labels is not None:
                ...
                logits = self.score(
                    hidden_states
                ).float()  # [bs, seq_len, 1] or [bs, seq_len, num_atoms]
                ...
                dtype, device = (
                    output_past_key_values[0][0].dtype,
                    output_past_key_values[0][0].device,
                )
                ...
                expanded_attention_mask = torch.cat(
                    [
                        attention_mask,
                        torch.ones(
                            (bs, top_k), dtype=torch.long, device=attention_mask.device
                        ),
                    ],
                    dim=1,
                )
                ...
                actual_position_ids = (
                    torch.ones((1, top_k)) * attention_mask.shape[1]
                ).to(dtype=attention_mask.dtype, device=device)
                ...
                diagonal_mask = diagonal_mask.to(
                    dtype=actual_attention_mask.dtype, device=device
                )
...
class CustomValueGuidedLogitProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        self.ref_model_tokenizer = (
            ref_model_tokenizer  # tokenizer for ref_model and value_classifier
        )
    ...
    def get_classifier_values(self, input_ids, top_k_indices):
        if self.classifier_state["first_pass"]:
            ...
            if not self.classifier_state["use_cache"]:
                ...
                input_ids = input_ids[
                    :, -1:
                ]  # due to cache, we only need the last token
            self.classifier_state["input_ids"] = input_ids
        ...
        if self.classifier_state["use_cache"]:
            ...
            self.classifier_state["past_key_values"] = (
                classifier_outputs.past_key_values
            )
    ...
    def modify_top_k_logits(self, ref_model_logits, logit_offset, top_k_indices):
        return torch.scatter_add(
            ref_model_logits, 1, top_k_indices.to(ref_model_logits.device), logit_offset
        )
    ...
    def __call__(self, input_ids, ref_model_logits):
        ...
        if self.modify_top_k == -1:
            top_k_indices = (
                torch.arange(ref_model_logits.size(-1))
                .unsqueeze(0)
                .expand(ref_model_logits.size(0), -1)
            )
        else:
            _, top_k_indices = torch.topk(ref_model_logits, self.modify_top_k, dim=-1)
        if self.loss_type == "mle":
            classifier_logits = self.get_classifier_values(
                input_ids, top_k_indices
            ).float()  # [bs, top_k, num_atoms]
            log_pmfs = F.log_softmax(
                classifier_logits, dim=-1
            )  # [bs, top_k, num_atoms]
            atoms = self.value_classifier.atoms.float().to(
                log_pmfs.device
            )  # [num_atoms]
            eps = 1e-6
            t = atoms.clamp(eps, 1.0 - eps)
            psi_atoms = torch.log(t / (1.0 - t))  # [num_atoms]
            logit_offset = torch.logsumexp(
                log_pmfs + self.eta * psi_atoms, dim=-1
            )  # [bs, top_k]
            combined_logits = self.modify_top_k_logits(
                ref_model_logits, logit_offset, top_k_indices
            )  # [bs, vocab_size]

        elif self.inference_mode in ["bernoulli", "expectation"]:
            classifier_logits = self.get_classifier_values(
                input_ids, top_k_indices
            ).float()
            if self.cd_baseline:
                ...
                z = torch.sigmoid(classifier_logits)
                eps = 1e-6
                z = z.clamp(eps, 1.0 - eps)
                psi_z = torch.log(z / (1.0 - z))
                logit_offset = psi_z * self.eta
            combined_logits = self.modify_top_k_logits(
                ref_model_logits, logit_offset, top_k_indices
            )
        else:
